...
//Relative path
let value:any[]=parse(fs.readFileSync("data/leafdata.csv"),{columns:true, skip_empty_lines:true})

for(let data of value){
test(`learn to read data from csv ${data.Testcaseid}`, async({page})=>{
    //login to page
  ...
  await page.locator('#createLeadForm_marketingCampaignId').selectOption({value:data.Marketingcampaign})
...
await page.locator('#createLeadForm_industryEnumId').selectOption({index:parseInt(data.Industry)})
//Select INR from the Preferred Currency dropdown
await page.locator('#createLeadForm_currencyUomId').selectOption({label:data.preferredcurrency})
